...
// Get space members (chat participants)
// IMPORTANTE: Esta ruta debe estar ANTES de router.get('/:id') para que funcione correctamente
router.get('/:id/members', async (req, res) => {
  try {
    ...
    if (!spaceChat) {
      // No chat yet - return empty members array
    ...
    const members = (participants || []).map(p => ({
      ...
      display_name: p.users?.name || p.users?.email || 'Unknown',
      avatar_photo: p.users?.avatar_photo || null,
      avatar_emoji: p.users?.avatar_emoji || null,
      avatar_color: p.users?.avatar_color || null,
      avatar_url: p.users?.avatar_photo || null
    }));
