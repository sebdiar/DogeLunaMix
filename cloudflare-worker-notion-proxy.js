...
// Notion JS Injection (aplica estilos dinámicamente + detecta cambios de URL)
const NOTION_JS = `
  (function() {
    // Aplicar estilos
    const apply = () => {
    ...
    // ===== DETECCIÓN DE CAMBIOS DE URL PARA POSTMESSAGE =====
    // Este script detecta cambios de URL en Notion y notifica al padre via postMessage
    // Esto permite que el historial funcione correctamente en el iframe
    
    let lastUrl = window.location.href;
    let isNotifying = false;
    
    // Función para notificar al padre sobre cambios de URL
    function notifyParent(url) {
      if (isNotifying) return; // Evitar notificaciones duplicadas
      isNotifying = true;
      
      try {
        window.parent.postMessage({
          type: 'notion-url-change',
          url: url
        }, '*');
    ...
    notifyParent(lastUrl);
    
    // 1. Escuchar eventos de navegación del navegador
    ...
    // 2. Escuchar cambios de hash (aunque Notion no los usa mucho)
    ...
    // 3. Interceptar pushState y replaceState (Notion usa estos para navegación SPA)
    const originalPushState = history.pushState;
    const originalReplaceState = history.replaceState;
    
    history.pushState = function(...args) {
      originalPushState.apply(history, args);
      // Usar setTimeout para asegurar que la URL ya cambió
      setTimeout(() => {
        const currentUrl = window.location.href;
        if (currentUrl !== lastUrl) {
          lastUrl = currentUrl;
    ...
    // 4. Interceptar clics en enlaces de Notion (fallback adicional)
    document.addEventListener('click', (e) => {
      const link = e.target.closest('a[href]');
      if (link && link.href) {
        // Verificar si es un enlace interno de Notion
        try {
          const linkUrl = new URL(link.href, window.location.href);
          if (linkUrl.hostname.includes('notion.so') || linkUrl.hostname.includes('notion.com')) {
    ...
    }, true); // Usar capture phase para capturar antes que Notion
    
    // 5. Verificar cambios periódicamente (fallback final)
    setInterval(() => {
      const currentUrl = window.location.href;
      if (currentUrl !== lastUrl) {
        lastUrl = currentUrl;
